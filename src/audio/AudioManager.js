...
export class AudioManager {
  ...
  bindEvents() {
    this.unsubscribers.push(
      ...
      this.eventBus.on('threat-detected', () => this.playSfx('alarmSuspicious')),
      this.eventBus.on('target_detected', () => this.playSfx('alarmSuspicious')),
      this.eventBus.on('hostile-identified', () => this.playSfx('alarmEnemy')),
      this.eventBus.on('intercept_started', () => this.playSfx('interceptStarted')),
      ...
      this.eventBus.on('reserve-launch-available', () => this.playSfx('uiClick', { volumeMultiplier: 0.7 })),
      this.eventBus.on('reserve-launched', () => this.playSfx('interceptStarted')),
      this.eventBus.on('weapon-fired', (event) => this.playSfx('weaponFire', { position: { x: event.x, y: event.y } })),
      this.eventBus.on('mag_fired', (event) => this.playSfx('weaponFire', { position: { x: event.x, y: event.y } })),
      ...
      this.eventBus.on('target_neutralized', (event) => this.playSfx('explosion', { position: { x: event.x, y: event.y } })),
      this.eventBus.on('collision-event', (event) => this.playSfx('explosion', { position: { x: event.x, y: event.y } })),
      this.eventBus.on('collision_occurred', (event) => this.playSfx('explosion', { position: { x: event.x, y: event.y } })),
      this.eventBus.on('interception-success', () => this.playSfx('interceptionSuccess')),
      this.eventBus.on('target_cleared_non_threat', () => this.playSfx('uiClick', { volumeMultiplier: 0.7 })),
      this.eventBus.on('mission-success', () => this.playSfx('interceptionSuccess')),
      this.eventBus.on('mission-failure', () => this.playSfx('alarmEnemy')),
      this.eventBus.on('mission_failed_protected_area_penetrated', () => this.playSfx('alarmEnemy')),
